...
import { Zap, Eye, FileVideo, ThumbsDown, UserCheck } from "lucide-react";
...
export default function ActivityPage() {
  ...
  const { activities, loading, error, activeTab, fetchActivities, setActiveTab } = useActivityStore();

  useEffect(() => {
    if (token) fetchActivities(token);
  }, [token, fetchActivities]);

  const filtered = useMemo(() => activities.filter((a) => a.activity_type === activeTab), [activities, activeTab]);
  ...
  const tabs = TABS.map((t) => ({ key: t.key, label: t.label, icon: t.icon, count: counts[t.key] ?? 0 }));
        ...
        {loading ? (
          ...
          <ul className="space-y-2">
            {filtered.map((item) => {
              ...
              return (
                <li key={item.id} className="flex items-center gap-3 p-3 rounded-lg bg-nebula-700/30 hover:bg-nebula-600/30 transition-colors">
                  <div className="w-9 h-9 rounded-full flex items-center justify-center shrink-0"
                    style={{ background: "rgba(0, 163, 196, 0.12)", border: "1px solid rgba(0, 163, 196, 0.2)" }}>
                    <TabIcon className="w-4 h-4 text-neon-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    {item.author ? (
                      <p className="text-sm font-medium text-star-200">
                        {activeTab === "ad_viewed" && <span className="text-star-500 mr-1">Ad from</span>}
                        <span className="text-neon-400">{item.author}</span>
                      </p>
                    ) : (
                      <p className="text-sm font-medium text-star-200">Marked as not interested</p>
                    )}
                    {item.details && (
                      <a href={item.details} target="_blank" rel="noopener noreferrer"
                        className="text-xs text-star-500 hover:text-neon-400 truncate block transition-colors">
                        {item.details}
                      </a>
                    )}
                  </div>
