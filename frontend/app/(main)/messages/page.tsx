...
export default function MessagesPage() {
  ...
  const activeMessages = useMemo(
    () =>
      messages
        .filter((m) => m.conversation_id === activeConversation)
  ...
  const handleConversationClick = (convId: string) => {
    if (!token) return;
    setActiveConversation(token, convId);
  };
        ...
        <div className="flex gap-4 h-96">
          ...
          <div className="flex-1 glass-card p-4 space-y-3">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className={`h-10 shimmer rounded-xl ${i % 2 ? "ml-auto w-1/2" : "w-2/3"}`} />
            ))}
          </div>
        </div>
        ...
        <div className="flex gap-4 h-[600px]">
          ...
          <div className="w-64 shrink-0 glass-card flex flex-col overflow-hidden">
            <div className="p-3 border-b border-neon-600/20">
              <p className="text-xs font-semibold text-star-500 uppercase tracking-wider">
            ...
            <div className="overflow-y-auto flex-1">
              {conversations.map((conv) => {
                ...
                const lastMsg = convMessages[convMessages.length - 1] ?? convMessages[0];
                return (
                  <button
                    key={conv.id}
                  ...
                  >
                    <div className="flex items-center gap-1.5">
                      ...
                      <span
                        className={`text-xs font-medium truncate ${isActive ? "text-neon-400" : "text-star-200"}`}
                      >
                        {fixInstagramEncoding(conv.participants) || conv.conversation_id}
                      </span>
                    </div>
                    {lastMsg && (
                      <p className="text-xs text-star-500 truncate pl-5">
                        {fixInstagramEncoding(lastMsg.content) || "Media"}
                      </p>
                    )}
                  </button>
                );
            ...
            {activeConv ? (
              <>
                <div className="p-4 border-b border-neon-600/20 flex items-center gap-2">
                  ...
                  <span className="font-medium text-star-200 text-sm">
                    {fixInstagramEncoding(activeConv.participants) || activeConv.conversation_id}
                  </span>
                  <span className="ml-auto stat-badge">{activeMessages.length} messages</span>
                </div>
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  {loadingMessages ? (
                    <div className="flex items-center justify-center h-full gap-2 text-star-500">
                      <Loader2 className="w-4 h-4 animate-spin" />
                      <span className="text-sm">Loading messages…</span>
                    </div>
                  ) : activeMessages.length === 0 ? (
                    <div className="flex items-center justify-center h-full text-star-500 text-sm">
                      ...
                      <div key={msg.id} className="flex flex-col gap-0.5">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-semibold text-neon-400">
                            {fixInstagramEncoding(msg.sender_name)}
                          </span>
                        ...
                        <div
                          className="glass-card px-3 py-2 max-w-xl text-sm text-star-200 rounded-xl"
                          style={{ background: "rgba(13, 24, 41, 0.9)" }}
                        >
                          {fixInstagramEncoding(msg.content) || (
                            <span className="italic text-star-500">Media / attachment</span>
                          )}
                        </div>
                      </div>
                    ))
              ...
              <div className="flex-1 flex items-center justify-center text-star-500 text-sm">
